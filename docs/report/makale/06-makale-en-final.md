...
This study experimentally investigates whether performance values measured for individual computer vision components remain valid when those components share a single hardware platform within an integrated system. The work concerns a surveillance system in which twenty concurrent camera streams are processed on a single mid-range laptop graphics processor (RTX 3070 Laptop, 8 GB). Object detection, multi-object tracking, pose estimation, facial expression classification, anomaly detection and aggressive behaviour estimation are combined within a single processing pipeline. End-to-end latency was measured as 378 ms (p50) and 652 ms (p95), while the analysed frame rate remained at 2.34 frames per second per camera. For violence detection, averaging the scores of a skeleton-based gradient boosting model and a raw-pixel three-dimensional convolutional model yielded an F1 score of 0.937 (0.917 after correcting for threshold selection bias); the source of this gain was shown to be that the error sets of the two models overlap by only 10%. For anomaly detection the fusion approach yielded an area under the curve of 0.869 (95% cluster bootstrap confidence interval 0.806 to 0.929), whereas a control series applying the same temporal smoothing to a single signal yielded 0.860, indicating that the gain originates from smoothing rather than from signal combination. The second contribution of the study lies on the measurement side: in 43 of the 87 entries in the development log, a performance or accuracy claim was refuted by a subsequent measurement. These refutations fall into three categories: the upper bound of a criterion lying below its target, a criterion threshold lying below the resolution of the measuring instrument, and the unit of observation failing to satisfy the independence assumption. On the security side, although a single entry point architecture had been established, the inventory of endpoints passing through that point had not been enumerated; as a result the observability interface was found to be externally accessible without authentication, and this was remedied.
