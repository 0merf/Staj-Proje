...
| Veri seti | Kameralar | İçerik | Kullanım amacı | Erişim |
|---|---|---|---|---|
| VIRAT Ground 2.0 (Oh vd., 2011) | cam-01 ile cam-08 | Sabit gözetim: otopark, kampüs, bina girişi | Anomali modülünün "normal" profilini öğrenmesi | https://viratdata.org |
| Oxford Town Centre (Benfold & Reid, 2011) | cam-09 | Yoğun yaya trafiği, yer gerçekli | Takip doğruluğu referansı | `[ERİŞİM ADRESİ]` |
| PETS 2009 (Ferryman & Shahrokni, 2009) | cam-10 ile cam-14 | Çok kameralı kalabalık, ani dağılma | Kalabalık ve dağılma kuralları | http://cs.binghamton.edu/~mrldata/pets2009 |
| UBI-Fights (Degardin & Proença, 2020) | cam-15 | Uzun süreli gözetim, şiddet olaylı | Erken uyarı, bağlamlı ölçüm | `[ERİŞİM ADRESİ]` |
| UR Fall Detection (Kwolek & Kepski, 2014) | cam-16 | Kontrollü düşme kayıtları | Düşme kuralı doğrulaması | `[ERİŞİM ADRESİ]` |
| RWF-2000 (Cheng vd., 2021) | cam-17 | Gözetim kaynaklı kavga klipleri | Saldırganlık modülü | `[ERİŞİM ADRESİ]` |
| CUHK Avenue (Lu vd., 2013) | cam-18, cam-19 | Kampüs gözetimi, kare düzeyli etiket | Anomali kontrolü ve testi | `[ERİŞİM ADRESİ]` |
| Pexels | cam-20 | Yakın plan yüz içeren sahneler | İfade modülü testi | https://www.pexels.com |

Veri kullanımına ilişkin bir not: Oxford Town Centre kayıtları kamuya açık bir alanda elde edilmiş gözetim görüntüleridir. Veri seti yalnızca yerel değerlendirmede kullanılmış, bu kaynaktan alınan kareler makalede şekil olarak yer almamıştır.
...
**İkincisi, aynı iskelet özniteliklerini elle belirlenen ağırlıklar yerine bir gradyan artırma modeline veren yaklaşımdır.** Öznitelikler aynıdır; değişen yalnızca bunların nasıl birleştirildiğidir.

**Üçüncüsü, kısa video kesitlerini doğrudan ham piksel olarak işleyen üç boyutlu bir evrişimli ağdır.** Bu yaklaşım iskelet çıkarımına hiç bağlı değildir ve dolayısıyla poz kestiriminin başarısız olduğu durumlarda da çalışabilmektedir.
...
*Tablo 13. Alarm kesinliği (Wilson %95 güven aralığı).*
...
Gelecek çalışma için dört yön öne çıkmaktadır. Birincisi, çıkarım işinin süreç içinde paralelleştirilmesidir. Süreç düzeyinde bölüştürme bu çalışmada denenmiş ve ölçülmüştür (Bölüm III.A.6); verimi yüzde on dokuz artırmış, gecikmeyi yaklaşık yarıya indirmiş, ancak sistem belleği kullanımını yüzde doksan sekize çıkardığı için üretimde etkinleştirilmemiştir. Bellek maliyetinin kaynağı, her sürecin model kopyalarını ve tamponlarını ayrıca tutmasıdır. Aynı kazancı bellek çoğaltmadan elde etmenin yolu, tek süreç içinde grafik işlemci ve işlemci aşamalarının birbiriyle örtüşecek biçimde ardışıklandırılmasıdır; bu, mevcut seri döngünün yeniden yazılmasını gerektirmektedir ve bu çalışmanın süresine sığmamıştır. İkincisi, erken uyarı ölçütünün anlamlı biçimde değerlendirilebilmesi için olay öncesi bağlam içeren bir veri setinin kullanılmasıdır; bu çalışmada kullanılan iki veri setinin de bu özelliği taşımadığı ölçülerek gösterilmiştir. Üçüncüsü, etiketlemenin birden çok değerlendiriciyle yapılması ve değerlendiriciler arası uyumun raporlanmasıdır. Dördüncüsü, sistemin gerçek ağ koşulları altında, gerçek kameralarla doğrulanmasıdır.
...
**Veri ve kod erişilebilirliği.** Çalışmada geliştirilen kaynak kod, ölçüm betikleri ve ölçüm çıktıları (JSON biçiminde) genel erişime açık bir depoda yayımlanmıştır: `[DEPO BAĞLANTISI]`. Kaynak videolar, ilgili veri setlerinin kendi lisans koşulları gereği depoya dâhil edilmemiş, erişim adresleri Tablo 1'de verilmiştir.
...
> **Yazara not (bu kutu makaleden silinecektir).** Aşağıdaki künyelerden tam olanlar, proje boyunca tutulan veri seti kayıtlarından (`data/_sources/SOURCE.md`) alınmıştır ve doğrulanmıştır. `[YAZAR ADI DOLDURULACAK]` işaretli satırlarda başlık ve erişim adresi doğrudur; yazar adı ile yıl, bağlantıdaki yayının ilk sayfasından alınarak tamamlanacaktır. Bu makalede hiçbir künye tahminle yazılmamıştır.
...
Cheng, M., Cai, K., & Li, M. (2021). RWF-2000: An open large scale video database for violence detection. *25th International Conference on Pattern Recognition (ICPR)*, 4183-4190. https://arxiv.org/abs/1911.05913
...
Savchenko, A. V. (2024). *EmotiEffLib: Efficient facial emotion recognition library* [Yazılım]. https://github.com/sb-ai-lab/EmotiEffLib `[YAZAR ADI DOLDURULACAK: depo sayfasındaki yazar/atıf bilgisini doğrulayın]`
...
Ultralytics. (2026). *YOLO26: Unified real-time end-to-end vision models* [Yazılım ve dokümantasyon]. https://docs.ultralytics.com/models/yolo26
...
### Şiddet ve saldırganlık tespiti

`[YAZAR ADI DOLDURULACAK]` (2024). RTVD-Net: Real-time violence detection based on pre-training of human skeleton images. *ACM International Conference Proceeding Series*. https://dl.acm.org/doi/fullHtml/10.1145/3638837.3638878

`[YAZAR ADI DOLDURULACAK]` (2026). DIFEM: Keypoint-interaction based feature extraction module for violence detection. *Signal, Image and Video Processing*. https://link.springer.com/article/10.1007/s11760-026-05229-1

`[YAZAR ADI DOLDURULACAK]` (2024). CUE-Net: Violence detection video analytics with spatial cropping, enhanced UniformerV2 and modified efficient additive attention. *arXiv:2404.18952*. https://arxiv.org/abs/2404.18952

`[YAZAR ADI DOLDURULACAK]` (2024). Streamlining video analysis for efficient violence detection. *arXiv:2412.02127*. https://arxiv.org/abs/2412.02127

### Video anomali tespiti

Zou, `[AD BAŞ HARFİ DOLDURULACAK]`, vd. (2026). Unlocking vision-language models for video anomaly detection via fine-grained prompting. *IEEE/CVF Winter Conference on Applications of Computer Vision (WACV)*. https://arxiv.org/abs/2510.02155

`[YAZAR ADI DOLDURULACAK]` (2025). EventVAD: Training-free event-aware video anomaly detection. *arXiv:2504.13092*. https://arxiv.org/abs/2504.13092

`[YAZAR ADI DOLDURULACAK]` (2025). Benchmarking compact vision-language models for clip-level surveillance anomaly detection under weak supervision. *Journal of Imaging*, 11(11), 400. https://www.mdpi.com/2313-433X/11/11/400

### Yüz ifadesi tanıma

Chang, `[AD BAŞ HARFİ DOLDURULACAK]`, vd. (2024). LibreFace: An open-source toolkit for deep facial expression analysis. *IEEE/CVF Winter Conference on Applications of Computer Vision (WACV)*.

`[YAZAR ADI DOLDURULACAK]` (2025). OpenFace 3.0: A lightweight multitask system for comprehensive facial behavior analysis. *arXiv:2506.02891*. https://arxiv.org/abs/2506.02891

### Çoklu nesne takibi

`[YAZAR ADI DOLDURULACAK]` (2023). ByteTrackV2: 2D and 3D multi-object tracking by associating every detection box. *arXiv:2303.15334*. https://arxiv.org/abs/2303.15334
